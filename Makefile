...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=blackCore \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=blackCore \
...
draw-deps:
	...
	@goviz -i ./cmd/blackCore -d 2 | dot -Tpng -o dependency-graph.png
...
#
# Run blackCore on docker
# Example Usage:
# 	make docker-build   ## Builds blackCore binary in 2 stages, 1st builder 2nd Runner
# 						   Final image only has the compiled blackCore binary
# 	make docker-interactive   ## Will start an shell session into the docker container
# 								 Access to blackCore binary here
...
release-build-platform:
	...
	$(MAKE) docker-build PROCESS="blackcore" DOCKER_FILE="Dockerfile.release" \
	...
	$(DOCKER) cp release-$(PLATFORM):/usr/local/app/build/blackCore release/blackCore-$(VERSION)-linux-$(PLATFORM)
	tar -zcvf release/blackCore-$(VERSION)-linux-$(PLATFORM).tar.gz release/blackCore-$(VERSION)-linux-$(PLATFORM)
	-@$(DOCKER) rm -f release-$(PLATFORM)
